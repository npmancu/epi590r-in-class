...
#Hold predictor constant (x) and vary the outcome
tbl_uvregression(
	nlsy,
	x = sex_cat,
...
#RR using poisson
logistic_model_RR_pois <- glm(glasses ~ eyesight_cat + sex_cat,
												 data = nlsy, family = poisson())
tbl_RR_pois <- tbl_regression(
	logistic_model_RR_pois,
	exponentiate = TRUE,
	label = list(
		sex_cat ~ "Sex",
		eyesight_cat ~ "Eyesight"),
...
tbl_merge(list(tbl_RR, tbl_RR_pois),
